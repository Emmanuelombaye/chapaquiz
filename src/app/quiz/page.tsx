...
export default function QuizEngine() {
  ...
  // Guard: redirect if not in a match
  useEffect(() => {
  ...
  const handleOptionClick = async (idx: number) => {
    ...
    await submitAnswer(currentQuestionIndex, idx);
  };

  // ----- Finished Screen -----
  if (matchStatus === 'finished') {
    const didWin = leaderboard.length > 0 && leaderboard[0]?.id === winnerId && userId === winnerId;
    ...
    return (
      <main className="p-6 flex flex-col h-full max-w-md mx-auto justify-center">
        <div className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 p-8 text-center flex flex-col items-center rounded-3xl shadow-sm">
          ...
          {didWin ? (
            <div className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 font-bold px-6 py-4 rounded-3xl mb-6 border border-green-500 w-full shadow-sm text-left">
              ...
              <div className="flex justify-between text-sm mb-1">
                <span>Total Pot ({leaderboard.length} players):</span>
                <span>KSh {totalPot}</span>
              </div>
              <div className="flex justify-between text-sm mb-2 text-green-600/80 dark:text-green-400/80">
                <span>Platform Fee (20%):</span>
                <span>-KSh {platformFee}</span>
              </div>
              <div className="flex justify-between text-lg border-t border-green-500/30 pt-2 mt-2 font-black">
                <span>Payout:</span>
                <span>+KSh {winnings}</span>
              </div>
            </div>
          ) : (
            <div className="bg-zinc-100 dark:bg-zinc-900/50 text-zinc-600 dark:text-zinc-400 font-bold px-6 py-4 rounded-3xl mb-6 border border-zinc-200 dark:border-zinc-700 w-full shadow-sm text-center">
              <p className="text-xl font-bold mb-1 text-zinc-800 dark:text-zinc-200">Better luck next time!</p>
              <p className="text-sm">Keep practicing to sharpen your skills.</p>
            </div>
          )}

          <p className="text-zinc-500 dark:text-zinc-400 mb-4 font-bold w-full text-left">Final Scores</p>
          ...
          <div className="w-full flex flex-col gap-3 mb-8">
            {leaderboard.map((p, i) => (
              <div key={p.id} className={clsx(
                ...
                i === 0
                  ? 'bg-yellow-50 dark:bg-yellow-500/10 border-2 border-yellow-500'
                  : 'bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800'
              )}>
                <div className="flex items-center gap-3">
                  <span className="text-xl w-6 text-left text-zinc-400">#{i + 1}</span>
                  <span className={clsx('text-zinc-900 dark:text-white', p.id === userId && 'text-green-600 dark:text-green-400')}>
                    {p.name} {p.id === userId ? '(You)' : ''}
                  </span>
                </div>
                <div className="flex gap-4">
                  <span className="text-blue-600 dark:text-blue-400">{Math.round(p.timeTaken)}s</span>
          ...
          <button
            onClick={() => { resetMatch(); router.push('/'); }}
    ...
    <main className="p-6 flex flex-col h-full max-w-md mx-auto">
      <header className="flex justify-between items-center mt-4 mb-8">
        <div className="bg-zinc-200 dark:bg-zinc-800 px-4 py-2 rounded-full font-bold text-sm text-zinc-800 dark:text-zinc-200">
          Q{currentQuestionIndex + 1} <span className="text-zinc-500">/ {questions.length}</span>
        </div>
        <div className={clsx(
          'px-4 py-2 rounded-full border-2 flex items-center gap-2 font-black text-lg tabular-nums',
          isTimeLow
            ? 'bg-red-100 border-red-500 text-red-600 dark:bg-red-900/30 dark:text-red-400 animate-pulse'
      ...
      {lastAnswerCorrect !== null && (
        <div className={clsx(
          'flex items-center gap-2 px-4 py-3 rounded-xl mb-4 font-bold text-sm transition-all',
          lastAnswerCorrect
          ...
          {lastAnswerCorrect ? <CheckCircle className="w-5 h-5" /> : <XCircle className="w-5 h-5" />}
          {lastAnswerCorrect ? 'Correct! +1 point' : 'Wrong answer'}
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex flex-col gap-3">
          {currentQ.options.map((opt, idx) => {
            ...
            const isLocked = localSelected !== null;
            ...
            return (
              <button
                ...
                className={clsx(
                  ...
                  isSelected
                    ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-600 dark:border-blue-500 text-blue-900 dark:text-blue-100'
                    : 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-800 dark:text-zinc-200 hover:border-blue-300 dark:hover:border-blue-700',
              ...
              >
                <div className={clsx(
                  ...
                  isSelected
                    ...
                    : 'bg-zinc-100 dark:bg-zinc-900 border-zinc-300 dark:border-zinc-600 text-zinc-500'
                )}>
                  {['A', 'B', 'C', 'D'][idx]}
                </div>
                {opt}
